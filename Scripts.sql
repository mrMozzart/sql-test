 ...
 */
WITH TAB AS 
(SELECT rfr.ITEM || rfr.LOCATION || rfr.ACTION_DATE str, RFR.selling_retail
FROM RPM_FUTURE_RETAIL rfr
GROUP BY rfr.ITEM, rfr.LOCATION, rfr.ACTION_DATE 
HAVING count(rfr.ITEM) > 1)
...
FROM RPM_FUTURE_RETAIL rfr 
WHERE rfr.SELLING_RETAIL IS NOT NULL AND rfr.ITEM NOT IN (SELECT DISTINCT ITEM
														  FROM RPM_ZONE_FUTURE_RETAIL rzfr
														  JOIN RPM_ZONE_LOCATION rzl ON rzfr.ZONE = rzl.ZONE_ID 
														  WHERE rzfr.SELLING_RETAIL IS NOT NULL AND rfr.LOCATION = rzl.LOCATION)
